    ...
    <tr>
      ...
      <td>
        <div class="payment-action-group">
          <button type="button" class="view-payment-btn" data-payment-id="${escapeAttribute(payment.id)}" title="View payment">
            <i class="fa-regular fa-eye"></i>
          </button>
          ${
            balance > 0
              ? `
                <button type="button" class="pay-payment-btn" data-transaction-id="${escapeAttribute(payment.transactionId)}" title="Pay remaining balance">
...
function createMobileItem(payment) {
  ...
  return `
    <article class="mobile-payment-item">
      ...
      <div class="mobile-payment-service">${escapeHtml(payment.service)}</div>
...
function openPaymentModal(payment) {
  ...
  body.innerHTML = `
    ...
    <div class="modal-total">
      ...
      <button type="button" class="modal-receipt-button" id="viewReceiptButton">
...
function printCurrentReceipt() {
  ...
  printWindow.document.write(`
    ...
    <head>
      <meta charset="UTF-8">
      <title>Payment Receipt - ${escapeHtml(payment.transactionId || "N/A")}</title>
      <style>
        * {
    ...
    <body>
      <div class="receipt">
        ...
        <div class="footer">
          <div>${escapeHtml(payment.notes || "Thank you for your payment.")}</div>
...
function renderPatientPaymentProcess() {
  ...
  const remaining = currentPayTransaction.balance;
  const amountToPay = Math.min(amount, remaining);
  if (method === "Cash") {
    wrapper.innerHTML = `
      <div class="payment-process-box">
        ...
        <div class="process-row">
          <div class="process-field">
            ...
            <div class="process-static-amount">${formatCurrency(amountToPay)}</div>
          </div>
          <div class="process-field">
            <label>Cash Given</label>
            <div class="process-info-value">
              <input type="number" id="patientCashReceived" min="0.01" step="0.01" placeholder="0.00">
            </div>
          </div>
          <div class="process-field">
            <label>Change</label>
            <div class="process-static-amount" id="patientCashChange">${formatCurrency(0)}</div>
          </div>
        </div>
        <button type="button" class="primary-button process-confirm-button" id="patientConfirmCash">
          Confirm Cash
        </button>
      </div>
    `;
    wrapper.hidden = false;
    const received = document.getElementById("patientCashReceived");
    const change = document.getElementById("patientCashChange");
    const button = document.getElementById("patientConfirmCash");
    if (received) {
      received.addEventListener("input", function () {
        const value = Number(received.value) || 0;
        if (change) {
          change.textContent = formatCurrency(Math.max(value - amountToPay, 0));
        }
        paymentProcessConfirmed = false;
        updatePatientPaySubmitState();
      });
    ...
    if (button) {
      button.addEventListener("click", function () {
        ...
        const actualPayment = Math.min(value, amountToPay);
        if (change) {
          change.textContent = formatCurrency(Math.max(value - amountToPay, 0));
        }
        paymentProcessConfirmed = actualPayment > 0;
        updatePatientPaySubmitState();
        showProcessConfirmed(button, "Cash Confirmed");
      });
    }
    return;
  }
    ...
    wrapper.innerHTML = `
      <div class="payment-process-box">
        ...
        <div class="qr-placeholder">
          ...
          <span>Scan the clinic's GCash QR code</span>
        </div>
        <div class="process-row">
          <div class="process-field">
            <label>Amount to Pay</label>
          ...
          <div class="process-field">
            ...
            <div class="process-info-value">
              <input type="text" id="patientGcashReference" placeholder="Enter GCash reference number">
            </div>
          </div>
        </div>
        <button type="button" class="primary-button process-confirm-button" id="patientConfirmGcash">
          Confirm GCash
        </button>
      </div>
    `;
    wrapper.hidden = false;
    attachReferenceProcess(
      "patientGcashReference",
      "patientConfirmGcash",
      "GCash",
    );
    return;
      ...
      <div class="payment-process-box">
        ...
        <div class="process-row">
          ...
          <div class="process-field">
            ...
            <div class="process-info-value">
              <input type="text" id="patientBankReference" placeholder="Enter transfer reference number">
            </div>
          </div>
        </div>
        <button type="button" class="primary-button process-confirm-button" id="patientConfirmBank">
          Confirm Transfer
        </button>
      </div>
    `;
      ...
      <div class="payment-process-box">
        ...
        <div class="process-row">
          ...
          <div class="process-field">
            ...
            <div class="process-info-value">
              <input type="text" id="patientCardReference" placeholder="Enter card authorization or reference number">
            </div>
          </div>
        </div>
        <button type="button" class="primary-button process-confirm-button" id="patientConfirmCard">
          Confirm Card
        </button>
      </div>
    `;
...
function submitPatientPayment() {
  ...
  const cashReceived =
    method === "Cash"
      ? Number(document.getElementById("patientCashReceived")?.value) || 0
      : amount;
  const paymentAmount =
  ...
  const now = new Date();
  const paymentId = `${transaction.id || "TXN"}-PAY-${history.length + 1}-${Date.now()}`;
  ...
  if (method === "GCash") {
    newHistoryPayment.gcashReference =
      document.getElementById("patientGcashReference")?.value.trim() || "";
  }
  if (method === "Bank Transfer") {
    newHistoryPayment.bankReference =
      document.getElementById("patientBankReference")?.value.trim() || "";
  }
  if (method === "Card") {
    newHistoryPayment.cardReference =
      document.getElementById("patientCardReference")?.value.trim() || "";
  }
...
function getPaymentReference() {
  ...
  if (method === "GCash") {
    return (
      document.getElementById("patientGcashReference")?.value.trim() || "N/A"
    );
  }
  if (method === "Bank Transfer") {
    return (
      document.getElementById("patientBankReference")?.value.trim() || "N/A"
    );
  }
  if (method === "Card") {
    return (
      document.getElementById("patientCardReference")?.value.trim() || "N/A"
    );
  }
  return "N/A";
}
